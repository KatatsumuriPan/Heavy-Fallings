...
public class ConfigHandler {
	...
	private static void create(ModConfigurationFile config, String categoryPath, @Nullable ConfigSide categorySide, @Nullable Object instance, Field field) throws IllegalArgumentException, IllegalAccessException {
		Class<?> type = field.getType();
		String id = getId(field);
		String fileComment = getCommentForFile(field);
		int order = getOrder(field);
		ConfigSide side = getSide(field, categorySide);
		if (type == boolean.class) {
			boolean default_value = field.getBoolean(instance);
			config.createBool(id, categoryPath, default_value, fileComment, order, side);
			...
			config.createInt(id, categoryPath, default_value, min, max, fileComment, order, side);
			...
			config.createLong(id, categoryPath, default_value, min, max, fileComment, order, side);
			...
			config.createFloat(id, categoryPath, default_value, min, max, fileComment, order, side);
			...
			config.createDouble(id, categoryPath, default_value, min, max, fileComment, order, side);
		} else if (type == char.class) {
			char default_value = field.getChar(instance);
			config.createChar(id, categoryPath, default_value, fileComment, order, side);
		} else if (type.isPrimitive()) {
			throw new RuntimeException("Not Supported:" + type.getName());
		} else if (type.isEnum()) {
			Enum<?> default_value = (Enum<?>) field.get(instance);
			config.createEnum(id, categoryPath, default_value, fileComment, order, side);
		} else if (type.isArray()) {
			throw new RuntimeException("Array not Supported");
		} else if (type == String.class) {
			String default_value = (String) field.get(instance);
			config.createString(id, categoryPath, default_value, fileComment, order, side);
		} else {
			...
			for (Field f : field.getType().getFields()) {
				create(config, new_category_path, side, field.get(instance), f);
			}
		}
	}
	...
	private static ConfigSide getSide(Field field, @Nullable ConfigSide defaultValue) {
		Side annotation = field.getAnnotation(Side.class);
		if (annotation != null)
			return annotation.value();
